...
if (length(args) < 2) {
   ...
   quit(save="no")
	  ...
	  install.packages(data.table, repo="https://cloud.r-project.org")
       ...
       emm <- fread(f, data.table=F, nThread=10, h=T, fill=T)
       bim <- fread(b,h=F, data.table=F, nThread=10, fill=T)
       bim <- fread(b,h=F, data.table=F, nThread=10, fill=T, col.names=c("CHR","SNP","cM","BP","A1","A2"))
       bp <- bim[,c(1:2,4)]
