...
for i in sm sma clean; do echo $i; done | parallel echo "--bfile imputed --recode 12 transpose --extract r2_0.5.snp.txt --geno 0.05 --hwe 1e-06 --keep {}.cov  --maf 0.05 --threads 30 --keep-allele-order --out {}" | xargs -I input -P5 sh -c "plink input"
